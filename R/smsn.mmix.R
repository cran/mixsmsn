...
smsn.mmix <- function(y, nu=1, mu = NULL, Sigma = NULL, shape = NULL, pii = NULL, g = NULL, get.init = TRUE, criteria = TRUE,
  ...
  if(get.init == FALSE){
       g <- length(mu)
       if((length(mu) != ncol(y)) || (length(mu) != length(Sigma)) || (length(mu) != length(shape)) || (length(mu) != length(pii))) stop("The size of the initial values are not compatibles.\n")
  ...
  if (get.init == TRUE){
    ...
    if(g > 1){
      ...
      for (j in 1:g){
        mu[[j]] <- init$centers[j,]
        shape[[j]] <- sign( apply( (y[init$cluster == j, ] - matrix(rep(mu[[j]], nrow(y[init$cluster == j, ])), nrow(y[init$cluster == j, ]), p, byrow = T  ) )^3, 2, sum ))
      ...
      mu[[1]] <- colMeans(y)
      Sigma[[1]] <- var(y)
      shape[[1]] <- sign( apply( (y - matrix(rep(mu[[1]], nrow(y)), nrow(y), p, byrow = T) )^3, 2, sum ))
