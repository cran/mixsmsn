  ...
  if (class(model) == "Skew.normal"){
    ...
    for (i in 1:n){
      ...
      for (j in 1:g){
        ...
        dPsi.dsigma11 <- (2/(2*pi)^(p/2))*( ddet.ds11*I.Phi(Ai=Ai, di=di) - (1/2)*dir.ds11*det(as.matrix(Sigma[j]))^(-1/2)*I.Phi(Ai=Ai, di=di) + det(as.matrix(Sigma[j]))^(-1/2)*dAir.ds11*I.phi(Ai=Ai, di=di) )
  ...
  if (class(model) == "Normal"){
    ...
    for (i in 1:n){
      ...
      for (j in 1:g){
        ...
        dPsi.dmu <- ((2*det(as.matrix(Sigma[j]))^(-1/2))/(2*pi)^(p/2))*( dAir.dmu * I.phi(di = di) - (1/2)*dir.dmu*I.Phi(di = di) )
        dPsi.dsigma11 <- (2/(2*pi)^(p/2))*( ddet.ds11*I.Phi(di = di) - (1/2)*dir.ds11*det(as.matrix(Sigma[j]))^(-1/2)*I.Phi(di = di) + det(as.matrix(Sigma[j]))^(-1/2)*dAir.ds11*I.phi(di = di) )
...
smsn.search <- function(y, nu, g.min = 1, g.max = 3, family = "Skew.normal", criteria = "bic", error = 0.0001, iter.max = 100, calc.im = FALSE)
{
 y <- as.matrix(y)
...
if(ncol(y) == 1){
  ...
  for(i in 1:length(g)){
     output[[g.size[i]]] <- smsn.mmix(y=y, nu=nu, g = g[i], get.init = TRUE, criteria = TRUE, group = TRUE, family = family, error = error, iter.max = iter.max)
     crit[i] <- output[[g.size[i]]][[criteria]]
  }
}
